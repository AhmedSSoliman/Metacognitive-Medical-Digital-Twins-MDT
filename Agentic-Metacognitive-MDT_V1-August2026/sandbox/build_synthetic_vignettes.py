"""
sandbox/build_synthetic_vignettes.py

CLI driver for sandbox/ehr_simulator.py.

PORTING NOTE (2026-08-12): ported from the source repo's
data/build_synthetic_vignettes.py, with the sys.path hack removed and
`from data.synthetic.vignette_generation import ...` rewritten to
`from sandbox.ehr_simulator import ...`. It travels with ehr_simulator.py
because it is that module's only entry point -- see ehr_simulator.py's
header for why the pair ended up under sandbox/. Usage lines below updated
for the new path.
...
sandbox/ehr_simulator.py's module docstring for the full
...
Usage:
    python -m sandbox.build_synthetic_vignettes
    python -m sandbox.build_synthetic_vignettes --target_total 1500 --seed 7
"""
...
from sandbox.ehr_simulator import backfill_forecasts, generate_archetype_vignettes
