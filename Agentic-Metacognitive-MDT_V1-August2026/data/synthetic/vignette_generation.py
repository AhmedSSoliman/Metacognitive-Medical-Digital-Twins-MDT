"""
data/synthetic/vignette_generation.py
...
1. All 36 original hand-written vignettes were missing the `forecast`
   ...
   defaulted to "not applicable" via training/sft_formatting.py's
...
2. Only 36 unique vignettes existed, requiring ~48x oversampling to reach
   an 8% target training-mix fraction (training/sft_trainer.py's
...
This module provides:
  - `backfill_forecasts(vignettes)`: adds a real forecast field to
    ...
    hypergraph/verification.py's flag-detection patterns, not fragile new
...
Forecast variable naming matches hypergraph/construction.py's
...
# hypergraph/verification.py's LearnedHypergraphChecker, so "what variable
...
# ---------------------------------------------------------------------------
# Theory-of-mind ground truth: what does THIS recipient already know?
#
# Deliberately generic (not archetype-specific vars) so this doesn't require
# a schema change to Archetype or its ~20+ existing instantiations. Three
# fact categories, present in every vignette regardless of clinical content:
#   - "the diagnosis/working assessment" -- clinicians always have this
#     (it's their job); patients/family may or may not, depending on
#     whether it's been formally discussed with them yet.
#   - "the raw numeric trend values" -- clinicians always have chart access;
#     patients/family are assumed NOT to have seen raw numbers unless a
#     clinician has walked them through it, which these vignettes don't
#     depict happening yet (this is a single momentary snapshot, not a
#     multi-turn conversation with an established history).
#   - "the prognosis/what happens next" -- nobody has this yet; it's what
#     THIS response is being asked to convey, so it's never in
#     recipient_knows for anyone (that would make the response pointless).
#
# core/rewards/theory_of_mind.py checks <user_belief> against this: does it
# avoid redundantly over-explaining what recipient_knows already covers,
# and does it correctly address (not silently skip) what
# recipient_does_not_know includes?
...
def _sample_recipient_knowledge_state(recipient_type: str, rng: random.Random) -> dict:
    """Returns {"recipient_knows": [...], "recipient_does_not_know": [...]},
    a partition of _KNOWLEDGE_FACTS. "the prognosis" is NEVER in
    recipient_knows for anyone (see module comment above)."""
    if recipient_type == "clinician":
        # Clinicians have chart access -- always know the diagnosis and raw
        # numbers already; only the forward-looking prognosis is genuinely
        # new information this response provides.
        return {
            "recipient_knows": ["the diagnosis or working clinical assessment", "the raw numeric trend values"],
            "recipient_does_not_know": ["what happens next / the prognosis"],
        }
    # Patients/family: whether the diagnosis has already been discussed with
    # them varies vignette-to-vignette (randomized); raw numbers are always
    # assumed not-yet-seen by this recipient class (they don't have chart
    # access), and prognosis is always new for everyone.
    diagnosis_already_told = rng.random() < 0.5
    knows = ["the diagnosis or working clinical assessment"] if diagnosis_already_told else []
    does_not_know = ["the raw numeric trend values", "what happens next / the prognosis"]
    if not diagnosis_already_told:
        does_not_know = ["the diagnosis or working clinical assessment"] + does_not_know
    return {"recipient_knows": knows, "recipient_does_not_know": does_not_know}
...
def generate_archetype_vignettes(n: int, seed: int = 1234) -> list[dict]:
    ...
    for i in range(n):
        ...
        knowledge_state = _sample_recipient_knowledge_state(recipient_type, rng)
        ...
        out.append({
            ...
            "recipient_knows": knowledge_state["recipient_knows"],               # R_tom ground truth
            "recipient_does_not_know": knowledge_state["recipient_does_not_know"],  # R_tom ground truth
        })
    return out
